...
<section class="turkey-map">
    <div class="page-header">
        <h1><?php echo $lang["tours_with_gallipoli"]?></h1>
    </div>
    <article class="tours">
        <div class="tour-box-two-column">
            <?php
            $turlar="includes/languages/lang.tours.".$SelectLang['code'].".php";
            include ($turlar);
            foreach ($toursHome as $value) {
                if (strstr($value["category"], ",")) {
                    $category_bol = explode(",", $value["category"]);
                    $category = trim($category_bol[1]);
                } else {
                    $category = $value["category"];
                }
                if ($category == "Tours With Gallipoli" OR $category == "Tours Con Gallipoli") {
                        ...
                        <div class="tour-inner">
                            ...
                            <div class="tour-images">
                                <a href="<?php echo $SelectLang["code"]."/".$value["link"];?>" title="<?php echo $value["title"];?>">
                                    <div class="golge"></div>
                                    <img  src="<?php echo $lang["SITE_URL"];?><?php echo str_replace("tours/","tours/detail/",$value["images"]); ?>" alt="<?php echo $value["title"];?>" loading="lazy" class="swiper-lazy">
                                </a>
                            </div>
